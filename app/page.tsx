      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-16">
        <div className="max-w-7xl px-2 lg:px-4">
          <p className="section-label mb-4 text-primary">
            Howrah, India &bull; Est. 1995
          </p>
          <h1 className="font-display font-bold text-5xl sm:text-6xl lg:text-8xl text-white leading-tight mb-6 text-balance">
            Precision Forging. <span className="text-primary">Strength</span>{" "}
            You Can Trust.
          </h1>
          <p className="text-lg text-white/75 leading-relaxed mb-10 max-w-2xl">
            Vikash Forge Pvt. Ltd. has delivered precision-engineered forgings
            to India&apos;s top industrial clients since 1995. Railways,
            automotive, and heavy engineering — we forge the components that
...
function StatsStrip() {
  const stats = [
    ...
    { value: "1L+", label: "Products Forged", icon: Cog },
    { value: "450+", label: "Industrial Clients", icon: Factory },
    { value: "18", label: "Heavy Machines", icon: Wrench },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="overflow-hidden">
          <Marquee speed={35} pauseOnHover gradient={false}>
            {/* Added a flex container with a proper gap instead of relying on margins */}
            <div className="flex gap-6 pr-6">
              {PRODUCTS.map((product, index) => (
                <div
                  key={`${product.slug}-${index}`}
                  className="w-[320px] h-[400px] shrink-0"
                >
                  <div className="group bg-card border border-border rounded overflow-hidden flex flex-col h-full shadow-sm">
                    ...
                    {/* Content Box with unified flex behavior */}
                    <div className="p-5 flex flex-col flex-1 min-h-0">
                      <h3 className="font-display font-bold text-lg text-foreground mb-2 truncate shrink-0">
                        {product.name}
                      </h3>

                      {/* Line clamp prevents long text from breaking your button positions */}
                      <p className="text-sm text-muted-foreground leading-relaxed mb-5 flex-1 overflow-hidden line-clamp-3">
                        {product.shortDesc}
                      </p>

                      {/* Action buttons pinned tightly to the bottom */}
                      <div className="flex gap-2 mt-auto shrink-0">
                        <Link
                          href={`/products/${product.slug}`}
                          className="flex-1 py-2.5 text-xs font-semibold text-center border border-border rounded hover:border-primary hover:text-primary transition-colors"
                        >
                          View Details
                        </Link>
                        ...
                        <Link
                          href="/contact"
                          className="flex-1 py-2.5 text-xs font-semibold text-center bg-primary text-primary-foreground rounded hover:bg-primary/90 transition-colors"
                        >
                          Send Enquiry
                        </Link>
                      </div>
        ...
        <div className="mt-12 flex justify-center items-center">
          {/* Using asChild prevents nesting a semantic <a> tag inside a <button> tag */}
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="order-1 lg:order-2">
            ...
            <p className="text-muted-foreground leading-relaxed mb-6 reveal delay-100">
              Founded in 1995 in Howrah, West Bengal, Vikash Forge Pvt. Ltd. has
              grown from a small forge shop into one of the region&apos;s most
              trusted open-die forging manufacturers. Our facility houses heavy
              hammers, manipulators, and precision machining equipment.
            </p>
  ...
  return (
    <section className="py-24 bg-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Added a fallback min-height to ensure the container never collapses to 0px */}
        <div className="overflow-hidden w-full min-h-[256px]">
          {MACHINES && MACHINES.length > 0 ? (
            <Marquee speed={35} direction="right" pauseOnHover gradient={false}>
              <div className="flex gap-6 pr-6">
                {MACHINES.slice(0, 4).map((machine, i) => (
                  <div
                    key={`${machine.id}-${i}`}
                    /* REMOVED: 'reveal' and 'delay-' classes to prevent opacity-0 lock */
                    className="w-[320px] h-64 shrink-0 group relative overflow-hidden rounded bg-card border border-border shadow-sm block"
                  >
                    <Image
                      src={machine.image}
                      alt={machine.name}
                      ...
                      sizes="320px"
                      className="object-cover transition-transform duration-500 group-hover:scale-105"
                      priority={i < 2} // Helps load the first couple items instantly
                    />
            ...
            /* Fallback loader state in case the MACHINES array is loading or empty */
            <div className="h-64 w-full flex items-center justify-center text-muted-foreground text-sm border border-dashed rounded">
              Loading machinery configurations...
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-8">
          <div>
            ...
            <h2 className="font-display font-bold text-2xl text-background">
              We work in EN9 &bull; EN19 &bull; EN24 &bull; DB6 &bull; EN8D
              &bull; CL4 &bull; CL2 &bull; EN31 &bull; EN42 &bull; EN4140
            </h2>
          </div>
          <Link
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          <div className="reveal-left">
            ...
            <p className="text-muted-foreground leading-relaxed mb-6">
              Tell us what you need — material grade, dimensions, quantity,
              application. Our technical team will respond within 24 business
